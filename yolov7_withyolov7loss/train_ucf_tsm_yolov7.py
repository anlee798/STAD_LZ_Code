...
from utils.misc import MyCollateFunc, build_dataset, build_dataloader
...
from models.yowo.yolo_loss import YOLOLoss
import numpy as np
...
def train():
    ...
    dataloader = build_dataloader(args, dataset, batch_size, MyCollateFunc(), is_train=True)
    ...
    if not os.path.exists('log'):
        os.mkdir('log')
    log_training = open(os.path.join('log', 'log.csv'), 'w')
    with open(os.path.join('log', 'args.txt'), 'w') as f:
        f.write("args:"+str(args))
        f.write('\n')
        
        f.write("m_cfg:"+str(m_cfg))
        f.write('\n')
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for iter_i, (frame_ids, video_clips, targets) in enumerate(dataloader):
            ...
            # inference
            # print("video_clips.device",video_clips.device)
            ...
            # loss
            images = video_clips[:, :, -1, :, :].to(device)    
            losses      = yolo_loss(outputs, targets, images)
            # print("yolo_loss",losses)
            ...
            tem = losses
            np_losses = tem.detach().cpu().numpy()[0]
            # print("np_losses",np_losses)
                    
            # Display
            if distributed_utils.is_main_process() and iter_i % 10 == 0:
                t1 = time.time()
                cur_lr = [param_group['lr']  for param_group in optimizer.param_groups]
                my_print_log(cur_lr, epoch,  max_epoch, iter_i, epoch_size,np_losses, t1-t0, accumulate,log_training,losses)
...
def my_print_log(lr, epoch, max_epoch, iter_i, epoch_size, loss, time, accumulate,log_training,losses_torch):
    # basic infor
    ...
    # loss infor
    log += '[{}: {:.2f}]'.format('losses', loss * accumulate)
    ...
    # other infor
    log += '[yolo_loss:'+str(losses_torch) +']'
    log += '[time: {:.2f}]'.format(time)

    # print log infor
    print(log, flush=True)
    log_training.write(log + '\n')
    log_training.flush()

def print_log(lr, epoch, max_epoch, iter_i, epoch_size, loss_dict, time, accumulate,log_training):
    # basic infor
    log =  '[Epoch: {}/{}]'.format(epoch+1, max_epoch)
    log += '[Iter: {}/{}]'.format(iter_i, epoch_size)
    log += '[lr: {:.6f}]'.format(lr[0])
