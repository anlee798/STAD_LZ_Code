...
# python dataset/my_ucf_jhmdb.py
...
class UCF_JHMDB_Dataset(Dataset):
    ...
    def pull_item(self, index):
        ...
        zeros = torch.zeros((target.shape[0], 1))
        target = torch.cat((target, zeros), dim=-1)
        target = torch.index_select(target, dim=1, index=torch.tensor([5, 4, 0, 1, 2, 3]))
...
#     dataloader = DataLoader(train_dataset,2,shuffle=False,collate_fn=MyCollateFunc())
...
#         import sys
#         sys.exit(0)
#     # build_dataloader(args, dataset, batch_size, CollateFunc(), is_train=True)
