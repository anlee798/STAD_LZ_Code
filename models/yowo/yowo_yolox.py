...
class YOWO(nn.Module):
    def __init__(self, 
        ...
        # outputs = self.backbone_2d(inputs)
        # print("type(outputs)",type(outputs))
        # print("bk_dim_2d",bk_dim_2d)
        
        ## 3D backbone
        self.backbone_3d, bk_dim_3d = build_backbone_3d(  #976
        ...
        self.reg_channel_encoders = nn.ModuleList(
            [build_channel_encoder(cfg, 4+bk_dim_3d, cfg['head_dim'])
                for i in range(len(cfg['stride']))])
    ...
    def forward(self,input):
        ...
        for level, (reg_feat, obj_feat, cls_feat) in enumerate(zip(reg_feats, obj_feats, cls_feats)):
            ...
            reg_feat = reg_feat.reshape(-1,int(math.sqrt(reg_feat.size(1))),int(math.sqrt(reg_feat.size(1))),reg_feat.size(2)).permute(0, 3, 1, 2)
            ...
            reg_pred = self.reg_preds[level](reg_feat)
            print("reg_pred.shape",reg_pred.shape)
