...
from ..backbone_2d_yolov7.myyolo import YoloBody
...
class YOWO(nn.Module):
    def __init__(self, 
        ...
        print("\n----------------------------------------------")
        print("Current Backbone YOWOv2_TSM_YOLOv7_Loss_YOWOv2")
        print("----------------------------------------------\n")
        ...
        if cfg['pretrained_2d']:
            ...
            if url is None:
                ...
                print('Loading 2D backbone pretrained weight: {}'.format("YOLOv7"))
        ...
        if cfg['pretrained_3d']:
            ...
            # state dict
            checkpoint = torch.load('/root/autodl-tmp/YOWOv2_TSM_yolov7_yolov8/Pretrain/ckpt.best.pth.tar',map_location=torch.device('cpu')) #load_state_dict_from_url(url, map_location='cpu')
            checkpoint_state_dict = checkpoint.pop('state_dict')
            # checkpoint_state_dict = checkpoint
    ...
    def forward(self, video_clips):
        ...
        if not self.trainable:
            ...
            # print("feat_3d:",feat_3d.shape)           #torch.Size([8, 464, 7, 7])
            # print("cls_feats[0]:",cls_feats[0].shape) #torch.Size([8, 64, 28, 28])
            # print("reg_feats[0]:",reg_feats[0].shape) #torch.Size([8, 64, 28, 28])
            # print("cls_feats[1]:",cls_feats[1].shape) #torch.Size([8, 64, 14, 14])
            # print("reg_feats[1]:",reg_feats[1].shape) #torch.Size([8, 64, 14, 14])
            # print("cls_feats[2]:",cls_feats[2].shape) #torch.Size([8, 64, 7, 7])
            # print("reg_feats[2]:",reg_feats[2].shape) #torch.Size([8, 64, 7, 7])
